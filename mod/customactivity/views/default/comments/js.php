...
elgg.comments.init = function() {
	...
	form.find('input[type=submit]').live('click', elgg.comments.submit);	
};

elgg.comments.submit = function(e) {
	...
	elgg.action('comments/add', {
		...
		success: function(json) {
			if(!riverId){		
				...
				var orderBy = elgg.trigger_hook('getOptions', 'comments.submit', json.output, 'asc');
				if (orderBy ==  'asc') {
					if (ul.length < 1) {
				...
				$.ajax({type: "GET",
					url: '<?php echo elgg_get_site_url()."ajax/view/comments/singleriver?id=";?>' + riverId,
					dataType: "html",
					cache: false,
