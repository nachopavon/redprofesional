...
<?php 
	...
	if(!empty($widgets)){
		...
		foreach($widgets as $handler => $widget){
			$can_add = widget_manager_get_widget_setting($handler, "can_add", $available_widgets_context);
			$allow_multiple = $widget->multiple;
			$hide = widget_manager_get_widget_setting($handler, "hide", $available_widgets_context);
