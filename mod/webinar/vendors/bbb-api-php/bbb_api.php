...
class BigBlueButton {
	...
	var $recorD; //record could be true or false
	...
	public function __construct() {
		...
		// pass the information to the class variables
			...
			$this->recorD = func_get_arg(5);
			$this->securitySalt = func_get_arg(6);
			$this->URL = func_get_arg(7);
			$this->logoutURL = func_get_arg(8);
	...
	*@return The url to join the webinar
	*/
	public function createWebinarURL($name, $meetingID, $attendeePW, $record ,$moderatorPW, $welcome, $logoutURL, $SALT, $URL ) {
		$url_create = $URL."api/create?";
		$voiceBridge = 70000 + rand(0, 9999);

		$params = 'name='.urlencode($name).'&meetingID='.urlencode($meetingID).'&attendeePW='.urlencode($attendeePW).'&record='.urlencode($record).'&moderatorPW='.urlencode($moderatorPW).'&voiceBridge='.$voiceBridge.'&logoutURL='.urlencode($logoutURL);

		if( trim( $welcome ) ) 
			$params .= '&welcome='.urlencode($welcome);
		echo "testing".$url_create.$params.'&checksum='.sha1("create".$params.$SALT);
	...
	*/
	public function createWebinarAndGetJoinURL( $username, $meetingID, $welcomeString, $mPW, $aPW, $rcD, $SALT, $URL, $logoutURL ) {

		$xml = bbb_wrap_simplexml_load_file( BigBlueButton::createWebinarURL($username, $meetingID, $aPW, $rcD, $mPW, $welcomeString, $logoutURL, $SALT, $URL ) );
	...
	*/
	public function elgg_createWebinarArray(ElggUser $user) {
		return $this->createWebinarArray( $user->name, $this->meetingID, $this->welcomeString, $this->modPW, $this->attPW, $this->recorD, $this->securitySalt, $this->URL, $this->logoutURL );
	}
	public function createWebinarArray( $username, $meetingID, $welcomeString, $mPW, $aPW, $rcD, $SALT, $URL, $logoutURL ) {
